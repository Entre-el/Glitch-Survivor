...
public class BubbleGunBrain : WeaponBrain
{
    ...
    private IEnumerator FireBurstCoroutine(Vector2 aimDirection)
    {
        ...
        {
            // 1. 组装当前发子弹的数据快照
            CombatPayload payload = SlotManager.GeneratePayload(
                0,
                baseDamage,
                basePierce,
                baseKnockback,
            ...
            if (payload.FireSticker != null)
            {
                ...
                payload.FireSticker.OnFireSlot(ref payload, emitter, finalDirection);
            }
            else
            {
                // 4. 如果没有开火贴纸，执行武器自带的、最原汁原味的发射逻辑
                GameObject bubbleObj = ObjectPoolManager.Instance.Get(
                    bubblePrefab,
                    spawnPos,
                    Quaternion.identity
                );
                if (
                    bubbleObj != null
                    && bubbleObj.TryGetComponent<ProjectileBase>(out var projectile)
                )
                {
